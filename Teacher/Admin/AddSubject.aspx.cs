    ...
    protected void btnsave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                {
                    conn.Open();
                    string Query = "select count(transId)+1 as SNo from tblSubject where status=1";
                    using (SqlCommand cmd = new SqlCommand(Query, conn))
                    {
                        using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                        {
                            ...
                            string Queryinsert = "insert into tblSubject(subjectCode,subjectName,addedby,addedDate,status) values('SC" + code + "','" + txtSubjectName.Text.Trim() + "','"+Session["userid"].ToString()+"',getdate(),1)";
                            using (SqlCommand cmdinsert = new SqlCommand(Queryinsert, conn))
                            {
                                ...
                                if (i > 0)
                                {
                                    ScriptManager.RegisterStartupScript(this, this.GetType(), "script", "alert('Subject Add Successfully');window.location='" + Request.ApplicationPath + "Admin/Subject.aspx';", true);
                                }
